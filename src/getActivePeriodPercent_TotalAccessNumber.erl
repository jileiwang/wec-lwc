-module (getActivePeriodPercent_TotalAccessNumber).
-export ([start/1, getGoodBlockPeriod/1,entrace/0]).

getPeriodLength() -> 10000.

getPeriodThrottling() -> 600.
...
getGoodBlockPeriod(Trace) ->
    erase(),
    put("average", 0),
    initPeriod(getPeriodThrottling()),
    GoodThrottling = 3,
    {ok, Fread} = file:open(getLogFile(Trace), read),
    L = loopBlocks(Fread, GoodThrottling, []),
    file:close(Fread),
    ListOrderByAccess = lists:reverse(lists:keysort(1, L)),
    {ok, LogFile} = file:open(getFilePath2() ++ "pop_percent_" ++ atom_to_list(Trace) ++ "_" ++ integer_to_list(GoodThrottling) ++ ".csv", write),
    lists:foreach(fun(X) -> {AccessSum, Percent} = X,
            io:format(LogFile, "~p,~p~n", [AccessSum, Percent]) end, ListOrderByAccess),
    file:close(LogFile),
    {ok, FAverage} = file:open(getFilePath2() ++ "average.csv", [append]),
    loopAverage(1, FAverage),
    file:close(FAverage).

loopAverage(N, FAverage) ->
    case get(N) =:= 0 of
        true ->
            Average = get("average") / (N - 1),
            io:format(FAverage, "~nAverage,~p~n", [Average]);
        _->
            PeriodAverageAccess = getPeriodLength() / get(N),
            put("average", get("average") + PeriodAverageAccess),
            io:format(FAverage, "~p,",[PeriodAverageAccess]),
            loopAverage(N + 1, FAverage)
...
loopBlocks(Fread, GoodThrottling, L) ->
    ...
    case Line of 
        ...
        _ ->             
            ...
            {Cache, _} = string:to_integer(Cache_str),
            {Sum, Percent} = loopPeriods(0, 0, 0, AccList, GoodThrottling, 0, 0),
            % io:format("~p",[0/0]),
            loopBlocks(Fread, GoodThrottling, [{Sum, Percent}|L])
    end.


loopPeriods(GoodPeriod, AllPeriod, _, [], _, Sum, _) ->
    {Sum, GoodPeriod/AllPeriod};
loopPeriods(GoodPeriod, AllPeriod, LiveSign, [H_str|L], GoodThrottling, Sum, Period) ->
    ...
    if
        LiveSign =:= 0 ->
            if
                H=:=0 ->
                    loopPeriods(GoodPeriod, AllPeriod, LiveSign, L, GoodThrottling, Sum, Period + 1);
                true ->
                    put(Period+1, get(Period+1) + 1),
                    if 
                        H >= GoodThrottling ->
                            loopPeriods(GoodPeriod + 1, AllPeriod + 1, 1, L, GoodThrottling, NewSum, Period + 1);
                        true ->
                            loopPeriods(GoodPeriod, AllPeriod + 1, 1, L, GoodThrottling, NewSum, Period + 1)
