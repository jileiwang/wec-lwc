...
initParamaters(WhitelistLen, WindowLen, WindowNum, Thre1, ThreMulti) ->  
  % ==== code for debug ===
  io:format("function called : initParamaters~n", []),
  % ===== end of debug ====
...
insertToWhiteList(CachelineID, WhiteList) ->
  ...
  case lists:member(CachelineID, WhiteList) of
    true ->
      WhiteList;
    _ ->
      if 
        length(WhiteList) >= WhitelistLen ->
          lists:sublist([CachelineID|WhiteList], WhitelistLen);
        true ->
          [CachelineID|WhiteList]
      end
  end.
...
addToArray(Array, CachelineID, WindowLen) ->
  % ==== code for debug ===
  io:format("function called : addToArray~n", []),
  % ===== end of debug ====
...
addtoLruLwcQueue(Q, Item, CacheCap, AddType) -> 
  % ==== code for debug ===
  io:format("function called : addtoLruLwcQueue~n", []),
  % ===== end of debug ====
